...
const SupplierDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      try {
        const d = await supplier.getSupplierById(Number(id));
        setSupplierData(d);
      } catch (error) {
        console.error('Erro ao buscar fornecedor:', error);
        showToastError('Erro ao carregar os detalhes do fornecedor');
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [id, supplier, showToastError]);

  const handleEdit = () => {
    navigate(`/panel/supplier/edit/${id}`);
  };

  const handleDelete = async () => {
    if (!supplierData) return;
    
    if (window.confirm(`Tem certeza que deseja excluir o fornecedor "${supplierData.name}"?`)) {
      try {
        setDeleting(true);
        await supplier.deleteSupplier(supplierData.id);
        showToastSuccess('Fornecedor excluído com sucesso! 🗑️');
        setTimeout(() => {
          navigate('/panel/supplier');
        }, 1500);
      } catch (error) {
        console.error('Erro ao excluir supplier:', error);
        showToastError('Erro ao excluir o supplier');
        setDeleting(false);
      }
    }
  };
        ...
        <Fade in timeout={500}>
          <Box>
            ...
            <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
              <Tooltip title="Voltar para lista de fornecedor">
                <IconButton
                  onClick={handleBack}
                  sx={{
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <Typography variant="body1" color="text.secondary" sx={{ mt: 0.5 }}>
                  Visualize todas as informações do fornecedor #{supplierData.id}
                </Typography>
              </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                      ...
                      <Box>
                        ...
                        <Typography variant="body1" sx={{ mt: 1, whiteSpace: 'pre-wrap' }}>
                          {supplierData.address || 'Sem enderço'}
                        </Typography>
                      </Box>
                        ...
                        <Grid item xs={12} sm={6}>
                          ...
                          >
                            ...
                            <Typography variant="body1" fontWeight="500">
                              {supplierData.neighborhood || 'Sem bairro'}
                            </Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={12} sm={6}>
                          ...
                          >
                            ...
                            <Typography variant="body1" fontWeight="500">
                              {supplierData.cityId || 'Não informado'}
                            </Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={12} sm={6}>
                          ...
                          >
                            ...
                            <Typography variant="body1" fontWeight="500">
                               {supplierData.stateId || 'Não informado'}
                            </Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={12} sm={6}>
